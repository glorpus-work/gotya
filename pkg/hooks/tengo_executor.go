...
func (e *TengoExecutor) Execute(hookType HookType, ctx HookContext) error {
	...
	if err := scriptInstance.Add("packageName", ctx.ArtifactName); err != nil {
		return fmt.Errorf("failed to add packageName to script: %w", err)
	}
	if err := scriptInstance.Add("packageVersion", ctx.ArtifactVersion); err != nil {
		return fmt.Errorf("failed to add packageVersion to script: %w", err)
	}
	if err := scriptInstance.Add("packagePath", ctx.ArtifactPath); err != nil {
