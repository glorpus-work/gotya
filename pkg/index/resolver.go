...
// PlatformMatch represents how well a pkg matches the target platform.
type PlatformMatch int

const (
	// NoMatch means the pkg doesn't match the platform at all.
	NoMatch PlatformMatch = iota
	// AnyMatch means the pkg matches but has "any" for either OS or Arch.
	AnyMatch
	// ExactMatch means the pkg exactly matches both OS and Arch.
	ExactMatch
)

// packageMatch represents a pkg with its platform match score.
type packageMatch struct {
	pkg   Package
	score PlatformMatch
}

// ResolvePackage finds the best matching pkg for the target platform.
func ResolvePackage(pkgs []Package, targetOS, targetArch string) (*Package, error) {
	if len(pkgs) == 0 {
		return nil, fmt.Errorf("no packages available")
	}

	// If no target platform is specified, use the first pkg
	...
	if len(matches) == 0 {
		return nil, fmt.Errorf("no matching pkg found for platform %s/%s", targetOS, targetArch)
	}
...
// getPlatformMatch determines how well a pkg matches the target platform.
func getPlatformMatch(pkg Package, targetOS, targetArch string) PlatformMatch {
	// If pkg has no platform specified, it matches any platform
	if pkg.OS == "" && pkg.Arch == "" {
		return AnyMatch
	}
