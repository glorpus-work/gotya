...
func TestTengoExecutor(t *testing.T) {
	executor := hook.NewTengoExecutor()
	...
	t.Run("Execute script with return value", func(t *testing.T) {
		...
		executor.AddScript(hook.PreInstall, script)

		err := executor.Execute(hook.PreInstall, ctx)
	...
	t.Run("Execute script with error", func(t *testing.T) {
		...
		`
		executor.AddScript(hook.PostInstall, script)

		err := executor.Execute(hook.PostInstall, ctx)
		assert.Error(t, err, "Execute should return an error for invalid script")
	})

	t.Run("Execute non-existent script", func(t *testing.T) {
		// Test executing a hook type that hasn't been added
		err := executor.Execute("non-existent-hook", ctx)
		assert.NoError(t, err, "Execute should not return an error for non-existent hook")
	})

	t.Run("HasScript check", func(t *testing.T) {
		// Test HasScript method
		hookType := hook.HookType("test-hook")
	...
	t.Run("Context variables are accessible", func(t *testing.T) {
		...
		`
		executor.AddScript(hook.PreRemove, script)

		err := executor.Execute(hook.PreRemove, ctx)
	...
	t.Run("Script can use basic operations", func(t *testing.T) {
		...
		`
		executor.AddScript(hook.PostRemove, script)

		err := executor.Execute(hook.PostRemove, ctx)
