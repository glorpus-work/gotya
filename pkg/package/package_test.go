...
// verifyPackageFiles verifies that all expected files exist in the package archive and have the correct content.
...
func verifyPackageFiles(t *testing.T, pkgPath string, expectedFiles []*testFile) error {
	...
	for {
		...
		if expected {
			...
			return fmt.Errorf("unexpected file found in package: %s", header.Name)
		}
	}
	...
	for pkgFilePath, _ := range expectedFileMap {
		if !foundFiles[pkgFilePath] {
			return fmt.Errorf("expected file not found in package: %s", pkgFilePath)
		}
	}
...
func TestCreatePackage(t *testing.T) {
	tests := []struct {
		...
		wantErr:     true,
		errContains: "invalid package structure",
	}, {
		...
		name: "referenced hook script missing",
		files: []*testFile{
			{path: "files/foo/bar.txt", content: "test content"},
		},
		hooks:       map[string]string{"post-install": "post-install.tengo"},
		...
		files: []*testFile{
			{path: "meta/package.json", content: "stuff"},
			{path: "files/foo/bar.txt", content: "test content"},
		},
		wantErr:     true,
		errContains: "invalid file in meta directory: package.json",
	}, {
